...
(function(){
const CFG = (window.EDEN_REVIEW_CONFIG) || {};
...
/* C17 · library/features/review-widget/commentable-everything.md → anchorEmpty.
   A tag listed in EDEN_REVIEW_CONFIG.anchorEmpty bypasses the direct-text gate
   ...
   ANCHOR_CELLS / window.EDEN_REVIEW_ANCHOR_ALL core-gate flag. */
 ...
 *
 * Contract:
 *   1. The page declares its dimensions in `window.EDEN_VARIANT_DIMENSIONS`,
...
function _dims(){ return window.EDEN_VARIANT_DIMENSIONS || {}; }
...
window.addEventListener('message', e => {
  if(!e || !e.data || e.data.type !== 'eden:lp-catalog') return;
...
function _slots(){ return (window.EDEN_SLOTS && Array.isArray(window.EDEN_SLOTS.declared)) ? window.EDEN_SLOTS.declared : []; }
function _broadcastPrefixes(){ return (window.EDEN_SLOTS && Array.isArray(window.EDEN_SLOTS.broadcastDefaultPrefixes)) ? window.EDEN_SLOTS.broadcastDefaultPrefixes : []; }
...
function safePrompt(msg,def){try{return window.prompt(msg,def);}catch(e){return null;}}
function reviewer(){let n=store.get('eden_reviewer');if(!n){n=(safePrompt(L.namePrompt,'')||'Anonymous').trim()||'Anonymous';store.set('eden_reviewer',n);}return n;}
...
function localAdapter(){
  const KEY='eden_review_comments'; let cb=null;
...
function render(){
   ...
   * on every LP whose EDEN_SLOTS.declared includes the slot AND whose SLUG
  ...
  rows.forEach(([id,c])=>{
    ...
    if(st==='pending'){
      ...
      acts.appendChild(actBtn(L.resolve,'resolve-btn',()=>{const note=(safePrompt(L.resolvePrompt,'')||'').trim();ADAPTER.update(id,{status:'resolved',resolution:note||('Resolved · '+ME),resolved_at:Date.now(),resolved_by:ME});}));
    } else {
...
(async function init(){
  ...
  /* variant-aware anchors (eden variation): the prototype rewrites a zone's
